...
>
> - **Claude Code** (the developer CLI) — its marketplace (`claude-plugins-official`) is the *full* catalog, auto-installed on startup and dominated by developer and infrastructure plugins: language servers, cloud (AWS/GCP/Azure/Oracle), databases, CI/CD, security, and code tooling. **~245 of the entries below run here.**
...
- [Pixeltable](https://docs.pixeltable.com) - Build multimodal AI applications with tables, computed columns, embedding search, UDFs, and tool-calling agents across 25+ AI provider integrations. *Use case: Multimodal data infrastructure, embedding-search pipelines, agent tool-calling over media tables.* (Claude Code)
...
- [Tavily](https://www.tavily.com) - Build AI applications with real-time web data using Tavily's search, extract, crawl, and research APIs. *Use case: Web-search grounding, structured extraction, research-agent data feeds.* (Claude Code)
...
- [Canva](https://www.canva.com) - Create, edit, review, resize, and brand-check Canva designs with the Canva MCP server. *Use case: Design generation from chat, brand-compliance checks, batch resizing for platforms.* (Claude Code)
...
- [Grafana Assistant](https://grafana.com/docs/grafana-cloud/assistant/) - Skills and rules for developing and using the Grafana Assistant app and CLI. *Use case: Grafana Assistant workflows, assistant-CLI automation.* (Claude Code)
- [Grafana Cloud MCP](https://grafana.com/products/cloud/) - Hosted MCP server for AI-assisted Grafana Cloud observability with no local installation required. *Use case: Grafana Cloud queries, hosted observability access, zero-install setup.* (Claude Code)
